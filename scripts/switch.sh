...
cd "$NIXOS_CONFIG_PATH" || { echo "Nixos config repo not found. Is NIXOS_CONFIG_PATH set?"; exit 1; }

# Build new NixOS generation
nixos-rebuild build --flake "$NIXOS_CONFIG_PATH"# || { echo "Error building NixOS generation"; exit 1; }
...
sudo nixos-rebuild switch --flake "$NIXOS_CONFIG_PATH"# || { echo "Error switching to new NixOS generation"; exit 1; }
